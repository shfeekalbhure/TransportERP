...
public static class Wave1ScreenApiModule
{
    public static IEndpointRouteBuilder MapWave1ScreenCatalog(this IEndpointRouteBuilder app)
    {
        ...
        {
            try { return Results.Ok(Wave1ScreenCatalog.GetRequired(screenId)); }
            catch (KeyNotFoundException) { return Results.NotFound(new { ErrorCode = "WAVE1_SCREEN_NOT_FOUND", ScreenId = screenId }); }
        });

        app.MapWave1Countries();
        app.MapWave1Geography();
        app.MapWave1AuthorizedNumbering();
        app.MapWave1ReferenceMasters();
        app.MapWave1AuthorizedAccounting();
