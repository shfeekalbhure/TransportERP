...
**Decision ID:** `DEC-P1-SYNC-ACTION-MAPPING-20260826-03`. ثُبتت القيم الحرفية التالية بسلطة القرارات المفوضة من المالك، من دون تفويض الدمج أو فتح Offline.

| ActionCode | OperationType | EntityType | EntityId | BaseVersion | ResultEntityId عند النجاح | Runtime availability على baseline |
|---|---|---|---|---|---|---|
...
- حقول challenge الصادرة `Id/CompanyId/RegisteredDeviceId/DeviceId/ChangeRequestId/ChangeType/ExpectedProofKeyVersion/NewProofKeyThumbprint/ChallengeHash/IssuedAt/ExpiresAt/CreatedByUserId` immutable بعد INSERT. التغيير الوحيد المسموح هو `ConsumedAt: NULL → timestamp` مرة واحدة، ويجب أن يكون `IssuedAt <= ConsumedAt < ExpiresAt` وأن يوجد قبل هذا التحديث change ledger مطابق لكل scope/change fields. يمنع إرجاع `ConsumedAt` إلى NULL أو تغييره بعد ضبطه، وبذلك لا يمكن تمديد challenge أو استبدال hash/key/scope أو استهلاكه بلا ledger.
...
10. migration preflight وlegacy collision والـpartial indexes وقيود INSERT وimmutable provenance، ورفض تعديل أي issued challenge field، ورفض استهلاك challenge بلا change ledger مطابق أو بعد انتهاء الصلاحية أو إعادة `ConsumedAt` إلى NULL، و`Up -> Down -> Up` على PostgreSQL الحقيقي، مع إثبات Down fail-closed عند وجود Stage 4 data.
