# BATCH-14 — Accounting Lifecycle & Control Screens — Design Authority
...
Older per-screen intelligence dossiers marked PARTIAL are working records and are not used to override the stronger current closed execution register and crosswalk.
...
| ACC-056 | القيود العكسية وربط القيد الأصلي بالقيد العاكس | Transaction | Reversal | 10 | 9 display columns (1 ordinal + 8 data) |
| ACC-057 | طلبات الاعتماد المحاسبية | ControlApproval | ApprovalQueue | 11 | No screen-specific business columns; Queue Grid/DetailsHost comes from shared ControlApproval foundation |
...
## Layout authority
Transaction variants reuse the shared Transaction host: `Toolbar → Header/MainData(Content) → Tabs/Workspace(Fill) → Detail/Grid(Fill where issued) → Actions/Totals(Content/Fixed) → Audit`.

ControlApproval foundation: decision header/reason = Content; approval/decision history and details use shared hosts. `ApprovalQueue` may use the shared Queue Grid/DetailsHost as Fill, but ACC-057 has no screen-specific column inventory to invent. `PeriodAction` has no concrete screen grid.
