...
public sealed class EfOrgRepository(TransportErpDbContext db) : IOrgRepository
{
    ...
    private static string Search(OrgEntity x) => x switch { Currency a => $"{a.Code} {a.ArabicName} {a.EnglishName}", Company a => $"{a.Code} {a.ArabicName} {a.EnglishName}", Branch a => $"{a.Code} {a.ArabicName} {a.EnglishName}", FiscalYear a => a.Code, NumberSequence a => $"{a.Code} {a.ArabicName} {a.EnglishName}", Language a => $"{a.LanguageCode} {a.ArabicName} {a.EnglishName}", _ => Code(x) };
    private static OrgDto ToDto(OrgEntity x) { var values=x.GetType().GetProperties().Where(p=>p.Name is not ("Id" or "IsActive" or "Version" or "CreatedAtUtc" or "UpdatedAtUtc" or "Code" or "ArabicName" or "EnglishName" or "LanguageCode")).ToDictionary(p=>p.Name,p=>p.GetValue(x)?.ToString()); return x switch { Currency a => new(a.Id,a.Code,a.ArabicName,a.EnglishName,a.IsActive,a.Version,values), Company a => new(a.Id,a.Code,a.ArabicName,a.EnglishName,a.IsActive,a.Version,values), Branch a => new(a.Id,a.Code,a.ArabicName,a.EnglishName,a.IsActive,a.Version,values), ExchangeRate a => new(a.Id,Code(a),"سعر صرف",a.Source,a.IsActive,a.Version,values), FiscalYear a => new(a.Id,a.Code,a.Code,a.Code,a.IsActive,a.Version,values), NumberSequence a => new(a.Id,a.Code,a.ArabicName,a.EnglishName,a.IsActive,a.Version,values), Language a => new(a.Id,a.LanguageCode,a.ArabicName,a.EnglishName,a.IsActive,a.Version,values), SettingOverride a => new(a.Id,Code(a),"","",a.IsActive,a.Version,values), _ => throw new ArgumentOutOfRangeException() }; }
}
