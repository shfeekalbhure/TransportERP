...
/// PostgreSQL triggers provide the database boundary; this interceptor is defense-in-depth
/// for tracked EF mutations.
/// </summary>
    ...
    private static void Reject<TEntity>(DbContext context, string label) where TEntity : class
    {
        ...
        if (ids.Length > 0)
            throw new InvalidOperationException(
                $"{label} is append-only; mutation denied: {string.Join(',', ids)}");
    }
}
