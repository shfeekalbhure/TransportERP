...
public sealed class EfShippingExecutionStore(TransportErpDbContext db, IWaybillAuditSink audit) : IShippingExecutionStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                var volume = (candidate.Item.Length ?? 0m) * (candidate.Item.Width ?? 0m) * (candidate.Item.Height ?? 0m);
                ManifestLines.Add(new ManifestLineEntity
                {
                    ...
                    Weight = candidate.Item.Weight ?? 0m, Volume = volume,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var lines = await ManifestLines.Where(x => x.ManifestId == manifestId).ToListAsync(cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        await audit.WriteAsync(context, "ManifestHandover", "SUCCESS", "Manifest", manifest.Id,
            null, JsonSerializer.Serialize(new { manifest.TripId, request.DriverId, request.AcceptedAt }), null, cancellationToken);
        await tx.CommitAsync(cancellationToken);
        return await ManifestResponseOf(context, manifest.Id, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var line in lines)
        {
            ...
            Movements.Add(new MovementEventEntity
            {
                ...
                ClientOperationId = DerivedMovementOperationId(operationId, line.Id)
            });
        }
    ...
    {
        var item = await Items.AsNoTracking().SingleOrDefaultAsync(x => x.Id == itemId, ct)
            ?? throw new WaybillPersistenceException("NOT_FOUND");
        var waybill = await Waybills.AsNoTracking().SingleOrDefaultAsync(x =>
            x.Id == item.WaybillId && (!waybillId.HasValue || x.Id == waybillId.Value) &&
            x.CompanyId == context.CompanyId && x.BranchId == context.BranchId, ct)
            ?? throw new WaybillPersistenceException("NOT_FOUND");
        return (waybill, item);
    }
    ...
    private async Task EnsureNoActiveHold(OperationContext context, Guid waybillId, CancellationToken ct)
    {
        if (await Holds.AsNoTracking().AnyAsync(x =>
            x.WaybillId == waybillId && x.CompanyId == context.CompanyId && x.BranchId == context.BranchId && x.Status == "ACTIVE", ct))
            throw new WaybillPersistenceException("HOLD_BLOCKED");
    }

    private async Task<decimal> ReleasedNet(Guid itemId, CancellationToken ct)
    {
        var entries = await Releases.AsNoTracking().Where(x => x.WaybillItemId == itemId).ToListAsync(ct);
        return entries.Sum(x => x.Status == ShippingExecutionStatuses.Release.Active ? x.Quantity : -x.Quantity);
    }

    private async Task<decimal> AllocationNet(Guid releaseId, CancellationToken ct)
    {
        var entries = await Allocations.AsNoTracking().Where(x => x.ReleaseId == releaseId).ToListAsync(ct);
        return entries.Sum(x => x.Status == ShippingExecutionStatuses.Allocation.Allocated ? x.Quantity : -x.Quantity);
    }

    private async Task<ItemQuantityStateResponse> ItemState(OperationContext context, Guid waybillId, Guid itemId, CancellationToken ct)
    {
        var scoped = await RequireItem(context, waybillId, itemId, ct);
        var released = await ReleasedNet(itemId, ct);
        return new ItemQuantityStateResponse(waybillId, itemId, scoped.Item.Quantity,
    ...
    {
        var trip = await Trips.AsNoTracking().SingleAsync(x => x.Id == tripId, ct);
        var stops = await Stops.AsNoTracking().Where(x => x.TripId == tripId).OrderBy(x => x.StopNo)
    ...
    private Task<ManifestLineResponse> ManifestLineResponseOf(Guid lineId, CancellationToken ct)
        => ManifestLines.AsNoTracking().Where(x => x.Id == lineId)
            .Select(x => new ManifestLineResponse(x.Id, x.AllocationId, x.WaybillId, x.WaybillItemId,
                x.Quantity, x.LoadedQuantity, x.Weight, x.Volume, x.LoadStatus)).SingleAsync(ct);
