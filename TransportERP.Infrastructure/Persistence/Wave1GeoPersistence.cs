...
public sealed class Wave1GeoDbContext(DbContextOptions<Wave1GeoDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder mb)
    {
        ...
        var governorate = mb.Entity<Governorate>();
        governorate.ToTable("governorates");
        governorate.HasIndex(x => x.CountryId);
        governorate.HasIndex(x => new { x.CountryId, x.Code }).IsUnique();
        governorate.HasIndex(x => new { x.CountryId, x.IsActive, x.Code });
        governorate.HasOne(x => x.Country)
            .WithMany(x => x.Governorates)
            .HasForeignKey(x => x.CountryId)
            .OnDelete(DeleteBehavior.Restrict);

        var directorate = mb.Entity<Directorate>();
        directorate.ToTable("directorates");
        directorate.HasIndex(x => x.GovernorateId);
        directorate.HasIndex(x => new { x.GovernorateId, x.Code }).IsUnique();
        directorate.HasIndex(x => new { x.GovernorateId, x.IsActive, x.Code });
        directorate.HasOne(x => x.Governorate)
        ...
        city.ToTable("cities");
        city.HasIndex(x => x.DirectorateId);
        city.HasIndex(x => new { x.DirectorateId, x.Code }).IsUnique();
        city.HasIndex(x => new { x.DirectorateId, x.IsActive, x.Code });
        city.HasOne(x => x.Directorate)
            .WithMany(x => x.Cities)
            .HasForeignKey(x => x.DirectorateId)
            .OnDelete(DeleteBehavior.Restrict);

        var area = mb.Entity<Area>();
        area.ToTable("areas");
        area.HasIndex(x => x.CityId);
        area.HasIndex(x => new { x.CityId, x.Code }).IsUnique();
        area.HasIndex(x => new { x.CityId, x.IsActive, x.Code });
        area.HasOne(x => x.City)
            .WithMany(x => x.Areas)
...
{
    public async Task<PagedResponse<GeoDto>> ListAsync(
        Wave1GeoResource resource,
        PagedQueryRequest query,
        CancellationToken ct = default)
    {
        ...
        return new PagedResponse<GeoDto>(
            rows.Skip((query.Page - 1) * query.PageSize)
                .Take(query.PageSize)
                .Select(ToDto)
                .ToArray(),
            query.Page,
    ...
    public async Task<GeoDto?> GetAsync(
        Wave1GeoResource resource,
        Guid id,
        CancellationToken ct = default)
        => (await Query(resource).AsNoTracking().FirstOrDefaultAsync(x => x.Id == id, ct)) is { } row
            ? ToDto(row)
            : null;

    public async Task<GeoDto> CreateAsync(
        Wave1GeoResource resource,
        object request,
        Wave1GeoOperationContext context,
        CancellationToken ct = default)
    {
        ...
        entity.Version = 1;
        db.Add(entity);
        await AppendAuditAsync(entity, "Create", null, context, null, ct);
        await db.SaveChangesAsync(ct);
        if (tx is not null) await tx.CommitAsync(ct);
        return ToDto(entity);
    }

    public async Task<GeoDto?> UpdateAsync(
        Wave1GeoResource resource,
        Guid id,
        object request,
        Wave1GeoOperationContext context,
        CancellationToken ct = default)
    {
        ...
        entity.Version++;
        await AppendAuditAsync(entity, "Update", before, context, null, ct);
        await db.SaveChangesAsync(ct);
        if (tx is not null) await tx.CommitAsync(ct);
        return ToDto(entity);
    }

    public async Task<GeoDto?> DisableAsync(
        Wave1GeoResource resource,
        Guid id,
        ...
        CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(request.Reason))
            throw new ArgumentException("Disable reason is required.");
        ...
        entity.Version++;
        await AppendAuditAsync(entity, "Disable", before, context, request.Reason.Trim(), ct);
        await db.SaveChangesAsync(ct);
        if (tx is not null) await tx.CommitAsync(ct);
        return ToDto(entity);
    }
    ...
    private async Task EnsureParentAndCodeAsync(
        Wave1GeoResource resource,
        GeoEntity entity,
        Guid? exceptId,
        CancellationToken ct)
    {
        ...
        if (!parentExists)
            throw new ArgumentException("PARENT_NOT_FOUND_OR_INACTIVE");
        ...
        var duplicate = resource switch
        {
            ...
            Wave1GeoResource.Governorates => await db.Governorates.AnyAsync(x => x.CountryId == parent!.Value && x.Code == entity.Code && x.Id != exceptId, ct),
            Wave1GeoResource.Directorates => await db.Directorates.AnyAsync(x => x.GovernorateId == parent!.Value && x.Code == entity.Code && x.Id != exceptId, ct),
            Wave1GeoResource.Cities => await db.Cities.AnyAsync(x => x.DirectorateId == parent!.Value && x.Code == entity.Code && x.Id != exceptId, ct),
            Wave1GeoResource.Areas => await db.Areas.AnyAsync(x => x.CityId == parent!.Value && x.Code == entity.Code && x.Id != exceptId, ct),
            _ => false
        };
        if (duplicate)
    ...
    private static void NormalizeAndValidate(GeoEntity entity)
    {
        ...
        if (string.IsNullOrWhiteSpace(entity.Code) || entity.Code.Length > 64)
            throw new ArgumentException("INVALID_CODE");
        if (string.IsNullOrWhiteSpace(entity.ArabicName) || entity.ArabicName.Length > 200)
            throw new ArgumentException("INVALID_ARABIC_NAME");
        if (entity.EnglishName?.Length > 200)
            throw new ArgumentException("INVALID_ENGLISH_NAME");

        if (entity is Country c)
        {
            c.NationalityName = string.IsNullOrWhiteSpace(c.NationalityName) ? null : c.NationalityName.Trim();
            if (c.NationalityName?.Length > 200)
                throw new ArgumentException("INVALID_NATIONALITY_NAME");
        }
    }
    ...
    {
        (Wave1GeoResource.Countries, CreateCountryRequest x) => new Country
        {
            Code = x.Code,
        ...
        (Wave1GeoResource.Governorates, CreateGovernorateRequest x) => new Governorate
        {
        ...
        (Wave1GeoResource.Directorates, CreateDirectorateRequest x) => new Directorate
        {
        ...
        (Wave1GeoResource.Cities, CreateCityRequest x) => new City
        {
        ...
        (Wave1GeoResource.Areas, CreateAreaRequest x) => new Area
        {
            CityId = x.CityId,
            Code = x.Code,
            ArabicName = x.ArabicName,
    ...
    private static void ApplyUpdate(Wave1GeoResource resource, GeoEntity entity, object request)
    {
        ...
        {
            case (Wave1GeoResource.Countries, Country a, UpdateCountryRequest b):
                a.Code = b.Code;
                a.ArabicName = b.ArabicName;
                a.EnglishName = b.EnglishName;
                a.NationalityName = b.NationalityName;
                break;
            case (Wave1GeoResource.Governorates, Governorate a, UpdateGovernorateRequest b):
                a.CountryId = b.CountryId;
                a.Code = b.Code;
                a.ArabicName = b.ArabicName;
                a.EnglishName = b.EnglishName;
                break;
            case (Wave1GeoResource.Directorates, Directorate a, UpdateDirectorateRequest b):
                a.GovernorateId = b.GovernorateId;
                a.Code = b.Code;
                a.ArabicName = b.ArabicName;
                a.EnglishName = b.EnglishName;
                break;
            case (Wave1GeoResource.Cities, City a, UpdateCityRequest b):
                a.DirectorateId = b.DirectorateId;
                a.Code = b.Code;
                a.ArabicName = b.ArabicName;
                a.EnglishName = b.EnglishName;
                break;
            case (Wave1GeoResource.Areas, Area a, UpdateAreaRequest b):
                a.CityId = b.CityId;
                a.Code = b.Code;
                a.ArabicName = b.ArabicName;
                a.EnglishName = b.EnglishName;
                break;
            default:
                throw new ArgumentException("REQUEST_RESOURCE_MISMATCH");
        }
    }
...
public sealed class Wave1GeographyMigration : Migration
{
    protected override void Up(MigrationBuilder m)
    {
        ...
        m.CreateIndex(
            ...
            unique: true);
        m.CreateIndex(
            name: "IX_countries_IsActive_Code",
            schema: "transport_erp",
            table: "countries",
    ...
    {
        m.DropTable(name: "areas", schema: "transport_erp");
        m.DropTable(name: "cities", schema: "transport_erp");
        m.DropTable(name: "directorates", schema: "transport_erp");
        m.DropTable(name: "governorates", schema: "transport_erp");
        m.DropTable(name: "countries", schema: "transport_erp");
    }
        ...
        string parentTable)
    {
        m.CreateTable(
            ...
            constraints: table =>
            {
                ...
                table.ForeignKey(
                    name: $"FK_{tableName}_{parentTable}_{parentColumn}",
                    column: x => x.ParentId,
        ...
        m.CreateIndex(
            name: $"IX_{tableName}_{parentColumn}",
            schema: "transport_erp",
            table: tableName,
            column: parentColumn);
        m.CreateIndex(
            name: $"IX_{tableName}_Parent_IsActive_Code",
            schema: "transport_erp",
            table: tableName,
            columns: new[] { parentColumn, "IsActive", "Code" });
        m.CreateIndex(
            name: $"UX_{tableName}_{parentColumn}_Code",
            schema: "transport_erp",
            table: tableName,
            columns: new[] { parentColumn, "Code" },
            unique: true);
    }
}
