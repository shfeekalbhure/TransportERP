...
The accepted TEAM-D v1.1 reconciliation contains 64 rows: 62 original records, `C1-CORR-001`, and `D-SEC-SYNC-001`. The reconciled snapshot contains ten projects in a flat `.slnx`. `TransportERP.Api` is the only source-composed executable. Application depends on Domain and Contracts; Infrastructure depends on Domain, Application, and Contracts; API depends on Application, Contracts, and Infrastructure; Desktop depends only on Contracts; Tests span API and lower layers; the three Mobile projects are isolated and source-empty. No ProjectReference cycle was proven. These are snapshot facts, not an authoritative-current-line judgment (`D11-EV-005`, `D11-EV-015`, `D11-EV-022`, `D11-EV-026`).

The following reconciled constraints govern this proposal:

- `A-ARCH-002 = CONFIRMED P0`: the registered Waybill repository deletes/reinserts items while `ToItemEntity` omits `Volume`; runtime reproduction and affected stored rows remain unknown (`D11-EV-006`, `D11-BLK-009`).
- `A-PRES-001 = CONFIRMED LOCAL-ONLY P0`: local-only heads/objects and dirty evidence must be preserved; their merge merit is unknown (`D11-EV-005`, `D11-BLK-007`).
- Identity is a claim-driven resource-server foundation; persistent request-time RBAC, session lifecycle, device registry, revocation, and Proof-of-Possession are unproven (`D11-EV-007..009`).
- Tenant isolation is manual/path-dependent and is not proven systemically at the database boundary (`D11-EV-007`, `D11-EV-009`).
- Offline/Sync is server enqueue/state foundation, not an end-to-end offline product (`D11-EV-013`). TEAM-D v1.1 additionally confirmed `D-SEC-SYNC-001 = P1`: four lifecycle methods apply tenant/general security but do not bind the stored operation's `UserId` and `DeviceId` to the caller; no current API exposure was proven (`D11-EV-014`, `D11-BLK-012`).
- Accounting posting, balanced journal creation, atomic audit, and complete database invariants are not implemented or proven (`D11-EV-010..012`).
- Desktop is a disconnected WinForms library; Mobile projects are placeholders (`D11-EV-015`).
- Shipping stops at partial execution; Ticketing and a reporting subsystem are absent on the assessed snapshot (`D11-EV-016`).
- `C1-CORR-001` corrects the prior factory claim: `TransportErpDbContextFactory` fails closed when `TRANSPORTERP_DESIGN_CONNSTR` is absent/blank and has no source-coded fallback; actual EF design-time execution and environment value remain unknown (`D11-EV-027`, `D11-BLK-013`).
- Exact-target runtime, live database, IdP/device, Production, deployment, recovery, latest Kurrasa authority, latest PR #69, external workspaces, Sync exposure/override policy, and EF tooling execution remain unknown (`D11-BLK-001..013`).
...
5. Every Sync lifecycle action must bind the stored operation owner (`UserId` + `DeviceId`) to the caller. A non-owner action is permitted only through a narrowly named privileged override, explicit authorization, reason, and immutable audit. Tenant equality alone is insufficient.
6. Defense in depth at service and database boundaries: tenant-consistent keys/FKs and an explicitly reviewed RLS or equivalent strategy. Exact choice is an ADR and DB-GOV-001 item because live roles/schema are unknown.
7. Bidirectional negative tests for company A/B and branch A/B, plus same-tenant cross-user and cross-device tests, across API, service, database, audit, export, reporting, sync, and background worker paths.
8. Sensitive-data classification, minimization, response redaction, export/print permissions, key-managed encryption where required, retention/legal hold rules, and log/telemetry scrubbing.
...
- operation-owner validation on transition, retry, conflict creation, and conflict resolution, with an explicitly modeled and audited privileged override where approved;
...
EF design-time tooling must remain fail-closed: the connection string is supplied explicitly through controlled non-secret environment provisioning, never by a source-coded fallback. A missing/blank value must fail before any connection attempt. Runtime success remains unproven until an isolated exact-SHA EF command is executed.
...
4. **Security/tenant gate:** implement and negatively test authoritative user/company/branch/device binding, Sync operation-owner binding/privileged override, and the approved DB defense strategy.
...
The proposal is complete within TEAM-C2's design scope, but it does not resolve evidence outside that scope. `UNKNOWN_AND_BLOCKERS_REGISTER.md` carries all thirteen TEAM-D v1.1 blockers plus target-design decisions. In particular:

- MASTER/GATE cannot assert final current state or readiness without an authoritative ref/full SHA.
- exact-target runtime, live DB, affected `Volume` rows, IdP/device, Sync lifecycle exposure/override policy, EF design-time execution, Production, privacy, deployment, and recovery evidence remain unknown.
