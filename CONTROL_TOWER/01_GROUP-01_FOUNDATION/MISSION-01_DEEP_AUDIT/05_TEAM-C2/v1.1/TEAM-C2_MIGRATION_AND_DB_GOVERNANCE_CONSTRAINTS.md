...
- Every Sync lifecycle mutation validates operation owner `UserId` and `DeviceId`; any privileged non-owner override is explicit, least-privilege, reasoned, and immutably audited.
...
- EF design-time configuration remains fail-closed. `TRANSPORTERP_DESIGN_CONNSTR` must be supplied explicitly from a controlled non-Production environment; no source fallback or secret is introduced. Actual tooling success requires isolated execution evidence.
...
| Offline | duplicate/reorder/replay/restart/conflict/revocation/schema-version plus same-tenant cross-user/cross-device lifecycle negatives and privileged-override audit tests |
