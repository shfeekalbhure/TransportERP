...
Before any transition/retry/conflict lifecycle mutation, the server must also compare the stored operation owner (`UserId` + `DeviceId`) with the caller. A non-owner path requires an explicit privileged override and immutable reasoned audit. This flow is disabled for business writes until the governing offline authority permits each operation.
