...
**Supersession:** Corrects and supersedes TEAM-C1 v1.0 for downstream use; v1.0 remains preserved and unchanged.
...
**Analytical source baseline:** `refs/heads/governance/control-tower-20260828` @ `8a36f88b56a43cd5b47277b645ba2030ed3da4f1`

**Correction evidence scope:** `TransportERP.Infrastructure/Persistence/TransportErpDbContextFactory.cs:8-18` at the full baseline SHA above; current control-worktree HEAD at reopen was `e2843caff509d34509146f9dfe2e748dea22df7e` and the cited source file was byte-identical between those commits.
...
**Status:** COMPLETE — v1.1 CORRECTED PACKAGE READY FOR CONTROL TOWER REVIEW
...
| Assessment source ref | `refs/heads/governance/control-tower-20260828` |
| Assessment source full SHA | `8a36f88b56a43cd5b47277b645ba2030ed3da4f1` |
| AUTHORITATIVE CURRENT LINE FOR THIS AUDIT | `UNKNOWN — REQUIRES VERIFICATION`; the assessment source ref is a fixed evidence snapshot, not an owner-authority designation |
...
- `TransportErpDbContextFactory.CreateDbContext` reads `TRANSPORTERP_DESIGN_CONNSTR`. If the value is null, empty, or whitespace, lines 11-15 throw `InvalidOperationException`; there is **no source-coded local fallback connection string** at the assessed baseline. When the environment variable is present, lines 16-18 pass it to `ConfigureTransportErpPostgreSql` and create the context.
...
### v1.1 correction boundary

- Corrected claim: the design-time factory fails closed when `TRANSPORTERP_DESIGN_CONNSTR` is absent; it does not fall back to a source-coded local connection string.
- Governing evidence: `C1-DATA-002` in `TEAM-C1_EVIDENCE_INDEX.md`.
- Register conformance: Source Access, Evidence, Files Reviewed, Unknown/Blockers, Domain Coverage, Workspace Preservation, Formation, Manifest, Seal, and Handoff were reissued for v1.1 with explicit mandatory fields.
- No other architectural determination was changed without new evidence.

#### C1-CORR-001 — Design-time factory fail-closed behavior

| Mandatory field | Value |
|---|---|
| Domain / specialty | Infrastructure / EF Core design-time / Database configuration |
| IMPLEMENTATION STATUS | `RUNTIME COMPLETE` for the static fail-closed branch in the source method; design-time command execution itself `UNKNOWN` |
| VERIFICATION STATUS | `VERIFIED` for source behavior; runtime environment `UNVERIFIED` |
| Temporal classification | `CURRENT` at baseline full SHA |
| OBSERVED FACT | `CreateDbContext` reads `TRANSPORTERP_DESIGN_CONNSTR`, throws when missing/whitespace, and configures only the supplied value. No source-coded fallback string exists in this file. |
| EVIDENCE | `C1-DATA-002`; file SHA-256 `d5c331d2180258fde574484de5f41a6ba78648743e0c7e3df68502620766c74c` |
| Project / file / class / method | `TransportERP.Infrastructure` / `Persistence/TransportErpDbContextFactory.cs:8-18` / `TransportErpDbContextFactory` / `CreateDbContext` |
| Migration / DB object | N/A; no Migration or Database object was modified or executed |
| Test / environment / result | Static direct-source verification only; EF command `NOT RUN` |
| Branch / full SHA | `refs/heads/governance/control-tower-20260828` / `8a36f88b56a43cd5b47277b645ba2030ed3da4f1` |
| Impact | Removes an unsupported credential/fallback architecture assertion from TEAM-C1 and preserves the actual fail-closed requirement. |
| Confidence | `HIGH` — entire source file inspected at exact SHA and matched by file hash |
| RECOMMENDATION | Keep environment provisioning and runtime success separate from the proven source fact; verify with synthetic non-Production configuration in a later authorized environment. |
| Priority | `INFO` — factual correction; no new remediation finding asserted |
| Not proved | Presence/value of the environment variable in any runtime, successful EF operation, or deployed configuration |
| Technical reviewer | TEAM-C1 data-architecture role |
| Affected-specialty reviewer | TEAM-C1 infrastructure/database role |
| Evidence reviewer | TEAM-C1 evidence role; single-session limitation recorded in formation register |
