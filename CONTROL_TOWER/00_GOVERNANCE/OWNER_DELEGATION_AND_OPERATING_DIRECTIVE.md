...
Control Tower is responsible for administering the full cycle until `GROUP-01` is closed. It remains a governance, verification, direction, sealing, stop, and handoff authority only and does not perform technical remediation.

The companion file-based supervision protocol is `CONTROL_TOWER/00_GOVERNANCE/CONTROL_TOWER_AUTONOMOUS_SUPERVISION_PROTOCOL.md`.
...
5. Record a completed team as `SEALED — DELIVERED TO CONTROL TOWER — STOP`; further modification requires `REOPEN`, a new version, and a new seal.
...
A P0 claim in an independent sealed input is not automatically a governing P0 determination and does not block TEAM-D from performing the read-only reconciliation expressly assigned by the master command. `OWNER DECISION REQUIRED` applies when the reconciled/confirmed condition blocks the affected gate, creates a preservation or destructive-action decision, or otherwise requires owner authority.
...
`NOT STARTED`, `ASSIGNED`, `WAITING`, `READY`, `IN PROGRESS`, `SEALED`, `STOPPED`, `HOLD`, `REOPENED`, `OWNER DECISION REQUIRED`.

Allowed current directives are:

`START`, `CONTINUE`, `WAIT`, `STOP`, `REOPEN`, `RETURN FOR REWORK`, `HOLD — OWNER DECISION REQUIRED`.

Every team or mission must read, in order, `CONTROL_TOWER/README.md`, this directive, its section in `CONTROL_TOWER_TEAM_DIRECTIVES.md`, its mission order, and the required sealed predecessor outputs before starting or resuming work. A team at `WAIT`, `HOLD`, or `STOP` must not work.

## 6. Output, seal, handoff, and archive rules

- Every team writes only inside its official directory. Chat is a session launcher, not a governing state store, and the owner is not required to copy reports between teams.
- A report is not closed until its main report, Evidence Index, Files Reviewed Register, Unknown/Blockers Register, applicable Domain/Architecture Coverage, Manifest, SHA-256, Seal Register, Handoff file, and all scope-closing conditions are verified.
- After that verification, the team disposition is `SEALED — DELIVERED TO CONTROL TOWER — STOP` and the central handoff may be recorded complete.
...
`DB-GOV-001` remains binding across every team and mission. No Database, Schema, Entity, Migration, field, or relationship change may be executed unless it is registered, reviewed, bound to an approved mission, supported by Impact Analysis and Preservation Requirements, and has a documented Test/Recovery path plus explicit execution authorization. Proposals are recorded for review before implementation.

## 8. Active-session monitoring cadence

While a Control Tower session remains actively able to monitor, it performs a workspace check every `10 minutes`. The check covers team directories, new reports, evidence registers, seal registers, handoff files, Task Queue, blockers, and mission states.

If no material change is found, Control Tower does not invent a decision or cosmetically rewrite records. If a real change is found, only the affected official records are updated.

Before active monitoring ends, `CONTROL_TOWER/04_CONTROL_TOWER_OPERATIONS/CONTROL_TOWER_LIVE_STATUS.md` must record `LAST VERIFIED CHECK`, `NEXT PLANNED CHECK`, and `MONITORING STATE`. If the session can no longer continue monitoring, it records exactly `MONITORING PAUSED — REQUIRES RESUME`; it must not claim ongoing monitoring.

## 9. Mandatory operating records

This directive is operated through and takes precedence as the direct owner delegation for the following records:

- `CONTROL_TOWER/04_CONTROL_TOWER_OPERATIONS/CONTROL_TOWER_LIVE_STATUS.md`
