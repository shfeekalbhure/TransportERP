...
public abstract class AccountingJournalReportForm : Form
{
    ...
    protected AccountingJournalReportForm()
    {
        ...
        StartPosition = FormStartPosition.CenterParent; MinimumSize = new Size(1120, 700);
    ...
    {
        var value = new ComboBox { Width = 150, DropDownStyle = ComboBoxStyle.DropDownList, RightToLeft = RightToLeft.Yes, BackColor = Color.FromArgb(255, 252, 219) };
        value.Items.AddRange(values); value.SelectedIndex = 0; return value;
    }

    private void BuildLayout()
    {
        var root = new TableLayoutPanel { Dock = DockStyle.Fill, RowCount = 5, Padding = new Padding(16), BackColor = BackColor };
        root.RowStyles.Add(new RowStyle(SizeType.Absolute, 62)); root.RowStyles.Add(new RowStyle(SizeType.Absolute, 48));
        root.RowStyles.Add(new RowStyle(SizeType.Absolute, 128)); root.RowStyles.Add(new RowStyle(SizeType.Percent, 100)); root.RowStyles.Add(new RowStyle(SizeType.Absolute, 36));
        root.Controls.Add(new Label { Text = "دفتر اليومية — ACC-021", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleRight, Font = new Font(Font, FontStyle.Bold), ForeColor = Color.FromArgb(35, 65, 100), BackColor = Color.White, Padding = new Padding(14) }, 0, 0);
        root.Controls.Add(BuildToolbar(), 0, 1); root.Controls.Add(BuildFilters(), 0, 2); root.Controls.Add(BuildGrid(), 0, 3); root.Controls.Add(BuildAudit(), 0, 4);
        Controls.Add(root);
    }
    ...
    {
        var panel = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.RightToLeft, WrapContents = true, Padding = new Padding(10), BackColor = Color.White };
        AddFilter(panel, "من تاريخ", _fromDate); AddFilter(panel, "إلى تاريخ", _toDate); AddFilter(panel, "الحساب", _account); AddFilter(panel, "الفرع", _branch);
        AddFilter(panel, "مركز التكلفة", _costCenter); AddFilter(panel, "الحالة", _status); AddFilter(panel, "العملة", _currency); AddFilter(panel, "نوع القيد", _journalType);
        _search.TextChanged += (_, _) => ApplyFilters(); _status.SelectedIndexChanged += (_, _) => ApplyFilters(); _journalType.SelectedIndexChanged += (_, _) => ApplyFilters(); _currency.SelectedIndexChanged += (_, _) => ApplyFilters();
        panel.Controls.Add(_search); panel.Controls.Add(_resultCount);
        return panel;
    }

    private static void AddFilter(FlowLayoutPanel panel, string caption, Control control) { panel.Controls.Add(new Label { Text = caption + ":", AutoSize = true, Padding = new Padding(5, 7, 2, 0) }); panel.Controls.Add(control); }
    ...
    private Control BuildGrid()
    {
        ...
        grid.Columns.AddRange(
            Column(nameof(JournalRow.Date), "التاريخ"), Column(nameof(JournalRow.Number), "رقم القيد"), Column(nameof(JournalRow.Account), "الحساب"),
            Column(nameof(JournalRow.Description), "البيان"), Column(nameof(JournalRow.Debit), "مدين"), Column(nameof(JournalRow.Credit), "دائن"), Column(nameof(JournalRow.Status), "الحالة"));
        return grid;
    }
    ...
    private void SeedRows() { _allRows.Add(new JournalRow(DateTime.Today.AddDays(-4), "JV-00041", "النقدية", "قيد يومي تجريبي", 125000M, 0M, "مرحّل", "YER", "قيد يومي")); _allRows.Add(new JournalRow(DateTime.Today.AddDays(-2), "JV-00042", "المصروفات", "تسوية محلية", 0M, 42500M, "معلق", "USD", "تسوية")); }
    ...
    private sealed record JournalRow(DateTime Date, string Number, string Account, string Description, decimal Debit, decimal Credit, string Status, string Currency, string JournalType);
}
