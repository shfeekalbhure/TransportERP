...
public abstract class AccountingJournalReportForm : Form
{
    ...
    private Control BuildFilters()
    {
        ...
        _search.TextChanged += (_, _) => ApplyFilters(); _fromDate.ValueChanged += (_, _) => ApplyFilters(); _toDate.ValueChanged += (_, _) => ApplyFilters(); _account.SelectedIndexChanged += (_, _) => ApplyFilters(); _branch.SelectedIndexChanged += (_, _) => ApplyFilters(); _costCenter.SelectedIndexChanged += (_, _) => ApplyFilters(); _status.SelectedIndexChanged += (_, _) => ApplyFilters(); _journalType.SelectedIndexChanged += (_, _) => ApplyFilters(); _currency.SelectedIndexChanged += (_, _) => ApplyFilters();
