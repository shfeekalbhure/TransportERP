...
public abstract class SetupDataFormBase : Form
{
    ...
    private void BuildLayout()
    {
        ...
        _fields.FlowDirection = FlowDirection.RightToLeft;
        _fields.WrapContents = true;
        ...
        foreach (var field in FieldNames)
        {
            ...
            _fields.Controls.Add(holder);
            _inputs[field] = input;
        }

        formBox.Controls.Add(_fields);
        root.Controls.Add(formBox, 0, 2);
        ...
        _audit.AutoSize = true;
        footer.Controls.Add(_audit);
    ...
    private void RefreshGrid()
    {
        ...
        // توحيد نوع طرفي التعبير الشرطي إلى List<SetupRecord> لمنع خطأ استنتاج النوع.
        List<SetupRecord> shown = string.IsNullOrWhiteSpace(query)
            ? _records.ToList()
            : _records
                .Where(x =>
        ...
        _grid.DataSource = new BindingList<SetupRecord>(shown);
        _counter.Text = $"عدد السجلات: {shown.Count}";
        _audit.Text = _currentIndex >= 0
            ? $"السجل الحالي: {_currentIndex + 1} | مرات التعديل: {_records[_currentIndex].EditCount} | آخر تعديل: {_records[_currentIndex].ModifiedAt:yyyy/MM/dd HH:mm}"
            : "سجل جديد";
    }
    ...
    private void NewRecord()
    {
        ...
        {
            _inputs[FieldNames[0]].Text = $"{ScreenCode}-{_nextId:000}";
        }
    ...
    private bool ValidateInputs()
    {
        foreach (var field in FieldNames)
        {
            if (!string.IsNullOrWhiteSpace(_inputs[field].Text))
            {
                continue;
            }

            _inputs[field].Focus();
            MessageBox.Show($"حقل «{field}» إلزامي.", ScreenTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return false;
        }
    ...
    private void SaveRecord()
    {
        ...
        {
            Code = _inputs[FieldNames[0]].Text.Trim(),
            Name = _inputs[FieldNames.Count > 1 ? FieldNames[1] : FieldNames[0]].Text.Trim(),
            Status = "نشط",
    ...
    private void EditRecord()
    {
        if (_currentIndex < 0)
        {
            MessageBox.Show("اختر سجلًا من الجدول أولًا.", ScreenTitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
        ...
        record.Code = _inputs[FieldNames[0]].Text.Trim();
        record.Name = _inputs[FieldNames.Count > 1 ? FieldNames[1] : FieldNames[0]].Text.Trim();
    ...
    private void ToggleStatus()
    {
        ...
        var record = _records[_currentIndex];
        record.Status = record.Status == "نشط" ? "موقوف" : "نشط";
        record.ModifiedAt = DateTime.Now;
        record.EditCount++;
        RefreshGrid();
    }
    ...
    private void DeleteRecord()
    {
        ...
        if (MessageBox.Show("هل تريد حذف السجل المحدد؟", ScreenTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes)
        {
            return;
        }
    ...
    {
        MessageBox.Show($"تم تجهيز بيانات {ScreenTitle} للطباعة.\nعدد السجلات الظاهرة: {_grid.Rows.Count}", "طباعة", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }

    private void LoadSelected()
    {
        if (_grid.CurrentRow?.DataBoundItem is not SetupRecord selected)
        {
            return;
        }
        ...
        foreach (var field in FieldNames.Skip(2))
        {
            if (string.IsNullOrWhiteSpace(_inputs[field].Text))
            {
                _inputs[field].Text = "—";
    ...
    private sealed class SetupRecord
    {
